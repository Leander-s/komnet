...
In der anderen Node messen wir genauso. Dann senden wir das Ergebnis and die 
Root-Node. Dort vergleichen wir, und wählen das kleinere Ergebnis, sodass
Wartezeiten zwischen Kommunikation wegfallen.
Etwa so:
\begin{lstlisting}
// In ping-exchange
void root_ping_exchange(..){
    ...
    MPI_Recv(otherLatency, ..);
    
    ..

    latency = min((end-start)/2, otherLatency);
}
...
Wir haben auch probiert, direkt die Half-round-trip Zeit zu messen, indem wir 
in der Root node den Sendestartpunkt und in der anderen Node den Punkt des
Nachrichterhalts messen. Leider sind die Uhren auf verschiedenen Systemen nicht
synchronisiert, sodass die Zeiten unbrauchbar waren.
\newpage
\section*{9.2}
